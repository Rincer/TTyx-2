...
SpvReflectAllocateType* pAllocateFunc = nullptr;
SpvReflectFreeType* pFreeFunc = nullptr;

void SpvReflectSetCustomMemoryManagement(SpvReflectAllocateType* pAllocate, SpvReflectFreeType* pFree)
{
    pAllocateFunc = pAllocate;
    pFreeFunc = pFree;
}

void* SpvReflectAllocate(size_t count, uint32_t size)
{
    void* res;
    if (pAllocateFunc == nullptr)
    {
        res = calloc(count, size);
    }
    else
    {
        res = pAllocateFunc(count, size);
    }
    memset(res, 0, count * size);
    return res;
}

void SpvReflectFree(void* p)
{
...
static SpvReflectResult IntersectSortedAccessedVariable(const SpvReflectPrvAccessedVariable* p_arr0, size_t arr0_size,
  ...
  *res_size = 0;
  ...
  if (*res_size > 0) {
    *pp_res = (uint32_t*)SpvReflectAllocate(*res_size, sizeof(**pp_res));
    if (IsNull(*pp_res)) {
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser* p_parser) {
  ...
  p_parser->nodes = (SpvReflectPrvNode*)SpvReflectAllocate(p_parser->node_count, sizeof(*(p_parser->nodes)));
  if (IsNull(p_parser->nodes)) {
    return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
  }
  ...
  if (p_parser->access_chain_count > 0) {
    p_parser->access_chains = (SpvReflectPrvAccessChain*)SpvReflectAllocate(p_parser->access_chain_count, sizeof(*(p_parser->access_chains)));
    ...
    switch (p_node->op) {
      ...
      case SpvOpSource: {
        ...
        if (p_node->word_count >= 5) {
          ...
          char* p_source_temp = (char*)SpvReflectAllocate(source_len + 1, sizeof(char));
      ...
      case SpvOpSourceContinued: {
        ...
        char* p_continued_source = (char*)SpvReflectAllocate(source_len + embedded_source_len + 1, sizeof(char));
      ...
      case SpvOpAccessChain: {
        ...
        if (p_access_chain->index_count > 0) {
          p_access_chain->indexes = (uint32_t*)SpvReflectAllocate(p_access_chain->index_count, sizeof(*(p_access_chain->indexes)));
...
static SpvReflectResult ParseStrings(SpvReflectPrvParser* p_parser) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    p_parser->strings = (SpvReflectPrvString*)SpvReflectAllocate(p_parser->string_count, sizeof(*(p_parser->strings)));
...
static SpvReflectResult ParseSource(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code)) {
    ...
    if (IsNotNull(p_parser->source_embedded)) {
      ...
      char* p_source = (char*)SpvReflectAllocate(source_len + 1, sizeof(char));
...
static SpvReflectResult ParseFunction(SpvReflectPrvParser* p_parser, SpvReflectPrvNode* p_func_node, SpvReflectPrvFunction* p_func,
  ...
  if (p_func->callee_count > 0) {
    p_func->callees = (uint32_t*)SpvReflectAllocate(p_func->callee_count, sizeof(*(p_func->callees)));
  ...
  if (p_func->accessed_variable_count > 0) {
    p_func->accessed_variables =
        (SpvReflectPrvAccessedVariable*)SpvReflectAllocate(p_func->accessed_variable_count, sizeof(*(p_func->accessed_variables)));
...
static SpvReflectResult ParseFunctions(SpvReflectPrvParser* p_parser) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    p_parser->functions = (SpvReflectPrvFunction*)SpvReflectAllocate(p_parser->function_count, sizeof(*(p_parser->functions)));
    ...
    for (size_t i = 0; i < p_parser->function_count; ++i) {
      ...
      p_func->callee_ptrs = (SpvReflectPrvFunction**)SpvReflectAllocate(p_func->callee_count, sizeof(*(p_func->callee_ptrs)));
...
static SpvReflectResult ParseMemberCounts(SpvReflectPrvParser* p_parser) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    for (uint32_t i = 0; i < p_parser->node_count; ++i) {
      ...
      p_node->member_names = (const char**)SpvReflectAllocate(p_node->member_count, sizeof(*(p_node->member_names)));
      if (IsNull(p_node->member_names)) {
        return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
      }

      p_node->member_decorations = (SpvReflectPrvDecorations*)SpvReflectAllocate(p_node->member_count, sizeof(*(p_node->member_decorations)));
...
static SpvReflectResult ParseDecorations(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  if (spec_constant_count > 0) {
    p_module->spec_constants = (SpvReflectSpecializationConstant*)SpvReflectAllocate(spec_constant_count, sizeof(*p_module->spec_constants));
...
static SpvReflectResult EnumerateAllUniforms(SpvReflectShaderModule* p_module, size_t* p_uniform_count, uint32_t** pp_uniforms) {
  ...
  *pp_uniforms = (uint32_t*)SpvReflectAllocate(*p_uniform_count, sizeof(**pp_uniforms));
...
static SpvReflectResult ParseType(SpvReflectPrvParser* p_parser, SpvReflectPrvNode* p_node,
  ...
  if (p_node->member_count > 0) {
    ...
    p_type->members = (SpvReflectTypeDescription*)SpvReflectAllocate(p_type->member_count, sizeof(*(p_type->members)));
...
static SpvReflectResult ParseTypes(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->_internal->type_descriptions = (SpvReflectTypeDescription*)SpvReflectAllocate(p_module->_internal->type_description_count,
  ...
  if (p_parser->physical_pointer_struct_count > 0) {
    p_parser->physical_pointer_structs = (SpvReflectPrvPhysicalPointerStruct*)SpvReflectAllocate(p_parser->physical_pointer_struct_count,
...
static SpvReflectResult ParseCapabilities(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->capabilities = (SpvReflectCapability*)SpvReflectAllocate(p_module->capability_count, sizeof(*(p_module->capabilities)));
...
static SpvReflectResult ParseDescriptorBindings(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->descriptor_bindings =
      (SpvReflectDescriptorBinding*)SpvReflectAllocate(p_module->descriptor_binding_count, sizeof(*(p_module->descriptor_bindings)));
...
static SpvReflectResult ParseDescriptorBlockVariable(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module,
  ...
  if (IsNotNull(p_type->members) && (p_type->member_count > 0)) {
    ...
    p_var->members = (SpvReflectBlockVariable*)SpvReflectAllocate(p_var->member_count, sizeof(*p_var->members));
...
static SpvReflectResult ParseInterfaceVariable(SpvReflectPrvParser* p_parser,
  ...
  if (p_type->member_count > 0) {
    p_var->member_count = p_type->member_count;
    p_var->members = (SpvReflectInterfaceVariable*)SpvReflectAllocate(p_var->member_count, sizeof(*p_var->members));
    if (IsNull(p_var->members)) {
      return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
    }
...
static SpvReflectResult ParseInterfaceVariables(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module,
  ...
  if (p_entry->input_variable_count > 0) {
    p_entry->input_variables =
        (SpvReflectInterfaceVariable**)SpvReflectAllocate(p_entry->input_variable_count, sizeof(*(p_entry->input_variables)));
  ...
  if (p_entry->output_variable_count > 0) {
    p_entry->output_variables =
        (SpvReflectInterfaceVariable**)SpvReflectAllocate(p_entry->output_variable_count, sizeof(*(p_entry->output_variables)));
  ...
  if (p_entry->interface_variable_count > 0) {
    p_entry->interface_variables =
        (SpvReflectInterfaceVariable*)SpvReflectAllocate(p_entry->interface_variable_count, sizeof(*(p_entry->interface_variables)));
...
static SpvReflectResult EnumerateAllPushConstants(SpvReflectShaderModule* p_module, size_t* p_push_constant_count,
  ...
  *p_push_constants = (uint32_t*)SpvReflectAllocate(*p_push_constant_count, sizeof(**p_push_constants));
...
static SpvReflectResult ParseStaticallyUsedResources(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module,
  ...
  if (called_function_count > 0) {
    p_called_functions = (uint32_t*)SpvReflectAllocate(called_function_count, sizeof(*p_called_functions));
  ...
  if (used_acessed_count > 0) {
    p_used_accesses = (SpvReflectPrvAccessedVariable*)SpvReflectAllocate(used_acessed_count, sizeof(SpvReflectPrvAccessedVariable));
    ...
    if (byte_address_buffer_offset_count > 0) {
      ...
      if (multi_entrypoint) {
        ...
        p_binding->byte_address_buffer_offsets =
            (uint32_t*)SpvReflectAllocate(byte_address_buffer_offset_count + p_binding->byte_address_buffer_offset_count, sizeof(uint32_t));
        ...
        p_binding->byte_address_buffer_offsets = (uint32_t*)SpvReflectAllocate(byte_address_buffer_offset_count, sizeof(uint32_t));
      }
...
static SpvReflectResult ParseEntryPoints(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->entry_points = (SpvReflectEntryPoint*)SpvReflectAllocate(p_module->entry_point_count, sizeof(*(p_module->entry_points)));
  ...
  for (size_t i = 0; entry_point_index < p_parser->entry_point_count && i < p_parser->node_count; ++i) {
    ...
    if (interface_variable_count > 0) {
      p_interface_variables = (uint32_t*)SpvReflectAllocate(interface_variable_count, sizeof(*(p_interface_variables)));
...
static SpvReflectResult ParseExecutionModes(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    ...
    uint32_t* indices = (uint32_t*)SpvReflectAllocate(p_module->entry_point_count, sizeof(indices));
    if (IsNull(indices)) {
      return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
    }
    for (size_t entry_point_idx = 0; entry_point_idx < p_module->entry_point_count; ++entry_point_idx) {
      ...
      if (p_entry_point->execution_mode_count > 0) {
        p_entry_point->execution_modes =
            (SpvExecutionMode*)SpvReflectAllocate(p_entry_point->execution_mode_count, sizeof(*p_entry_point->execution_modes));
...
static SpvReflectResult ParsePushConstantBlocks(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  p_module->push_constant_blocks =
      (SpvReflectBlockVariable*)SpvReflectAllocate(p_module->push_constant_block_count, sizeof(*p_module->push_constant_blocks));
...
static SpvReflectResult ParseEntrypointDescriptorSets(SpvReflectShaderModule* p_module) {
  ...
  for (uint32_t i = 0; i < p_module->entry_point_count; ++i) {
    ...
    if (p_entry->descriptor_set_count > 0) {
      p_entry->descriptor_sets = (SpvReflectDescriptorSet*)SpvReflectAllocate(p_entry->descriptor_set_count, sizeof(*p_entry->descriptor_sets));
    ...
    for (uint32_t j = 0; j < p_module->descriptor_set_count; ++j) {
      ...
      p_entry_set->bindings = (SpvReflectDescriptorBinding**)SpvReflectAllocate(count, sizeof(*p_entry_set->bindings));
...
static SpvReflectResult ParseDescriptorSets(SpvReflectShaderModule* p_module) {
  ...
  for (uint32_t i = 0; i < p_module->descriptor_set_count; ++i) {
    ...
    p_set->bindings = (SpvReflectDescriptorBinding**)SpvReflectAllocate(p_set->binding_count, sizeof(*(p_set->bindings)));
...
static SpvReflectResult CreateShaderModule(uint32_t flags, size_t size, const void* p_code, SpvReflectShaderModule* p_module) {
...
#ifdef __cplusplus
  p_module->_internal = (SpvReflectShaderModule::Internal*)SpvReflectAllocate(1, sizeof(*(p_module->_internal)));
#else
  p_module->_internal = SpvReflectAllocate(1, sizeof(*(p_module->_internal)));
#endif
  ...
  if (flags & SPV_REFLECT_MODULE_FLAG_NO_COPY) {
    ...
    p_module->_internal->spirv_size = size;
    p_module->_internal->spirv_code = (uint32_t*)SpvReflectAllocate(1, (uint32_t)p_module->_internal->spirv_size);
    p_module->_internal->spirv_word_count = (uint32_t)(size / SPIRV_WORD_SIZE);
